...
12. **`supabase/migrate_add_prospect_contact_fields.sql`** — `prospects`의 기존
    `category`(접촉경로와 "미접촉" 상태를 함께 나타내던 필드)를 `contacted`(접촉유무,
    boolean)와 `category`(접촉경로, 소개/골프/모임/교회/전화·이메일 연락/CPA·변호사
    /광고·명함/기타 8종 고정 분류)로 분리합니다. 기존 자유 텍스트 값은 새 8종 분류에
    최대한 자동 매칭되고, 매칭이 안 되는 값은 "기타"로 분류됩니다 — "모임/골프"처럼
    두 값이 합쳐진 기존 텍스트는 "골프"로 처리되므로, 실제로는 "모임" 접촉이었던
    특정 잠재고객이 있다면 Potential Client 탭에서 직접 수정해 주세요.
13. **`supabase/migrate_add_prospect_sales_priority.sql`** — `prospects`에
    Sales Priority(영업 우선순위) 컬럼을 추가합니다. A(우선영업)/B(접촉유지)/
    C(관망대상) 중 하나를 잠재고객 정보 입력/수정 화면에서 고를 수 있고, 필터
